...
public class RegionWME extends WME {
	...
	public static ArrayList<RegionWME> getRegions(String regionData) {
		...
		for (String location : locs) {
			...
			/*
			 * region.x = new int[region.numPoints]; region.y = new
			 * int[region.numPoints];
			 * 
			 * for (int i=0; i<region.numPoints; i++) { region.x[i] =
			 * Integer.parseInt(coords[5 + 2*i]); region.y[i] =
			 * Integer.parseInt(coords[6 + 2*i]); }
			 */
