 ...
 * resolution than the size tile in this class. Each tile is actually a 4x4
 * grid, but this has been abstracted away for simplicity for now.
 */
public class MapWME extends WME {
	...
	public boolean isBuildable(int tx, int ty, int width, int height) {
		...
		if (tx < 0 || ty < 0 || (tx + width) >= mapWidth
				|| (ty + height) >= mapHeight) {
			return false;
		}
	 ...
	 * 
	 * @param mapData
	 *            - mapname:width:height:data
	 * 
	 *            Data is a character array where each tile is represented by 3
	 *            characters, which specific height, buildable, walkable.
	 */
