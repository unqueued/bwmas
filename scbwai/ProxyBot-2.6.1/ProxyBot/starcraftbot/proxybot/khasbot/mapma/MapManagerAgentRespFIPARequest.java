...
public class MapManagerAgentRespFIPARequest extends AchieveREResponder {
	Agent agent=null;	
  MessageTemplate mt = null;

  public MapManagerAgentRespFIPARequest(Agent a, MessageTemplate mt) {
    super(a, mt);
    agent=a;
    this.mt=mt;
  ...
  protected ACLMessage handleRequest(ACLMessage request) throws NotUnderstoodException, RefuseException {
    ...
    } else if(ParseACLMessage.isSenderBattleManager(request)) {
      
      //
      // Here we have to determine what it is that the request is asking
      //

      //TODO: insert code to parse out what the BattleManager wants us to do. May have to define ontologies
      //still not sure

      ACLMessage agree = request.createReply();
      agree.setPerformative(ACLMessage.AGREE);
      return agree;
    } else if(ParseACLMessage.isSenderResourceManager(request)) {
      
      //
      // Here we have to determine what it is that the request is asking
      //

      //TODO: insert code to parse out what the ResourceManager wants us to do. May have to define ontologies
      //still not sure

      ACLMessage agree = request.createReply();
      agree.setPerformative(ACLMessage.AGREE);
      return agree;
    } else if(ParseACLMessage.isSenderBuildingManager(request)) {
      
      //
      // Here we have to determine what it is that the request is asking
      //

      //TODO: insert code to parse out what the BuildingManager wants us to do. May have to define ontologies
      //still not sure
